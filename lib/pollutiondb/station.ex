...
defmodule Pollutiondb.Station do
  ...
  def get_by_id(id) do
    Pollutiondb.Repo.get_by(Pollutiondb.Station, id: id)
  end
  ...
  def find_by_location_range(lon_min, lon_max, lat_min, lat_max) do
      query = from(s in Pollutiondb.Station,
         where: s.lon > ^lon_min and
                s.lon < ^lon_max and
                s.lat > ^lat_min and
                s.lat < ^lat_max)
  ...
  def validate_coordinates(changeset) do
    changeset
  ...
  def changeset(station, changesmap) do
    ...
    |> Ecto.Changeset.unique_constraint(:name, message: "Station name must be unique")
    |> Ecto.Changeset.unique_constraint([:lon, :lat], message: "Station already exists at this location")
end
end
